...
fn all_cases() {
    ...
    for line in lines {
        ...
        if parts.len() == 3 {
            expected = FriendlyProbability::new(numerator, denominator, String::from(""), None);
        } else if parts.len() == 4 {
            expected = FriendlyProbability::new(numerator, denominator, String::from(""), parts[3].to_string());
        } else {
